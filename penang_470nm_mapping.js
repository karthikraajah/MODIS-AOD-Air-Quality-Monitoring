...
function maskAOD(image) {
  ...
  var aod470 = image.select('Optical_Depth_047').multiply(0.001); // apply scale
  return aod470.updateMask(qaHigh).updateMask(clear);
}

// === 4. Month names for export ===
var monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];

// === 5. Loop through months (2015 only) ===
ee.List.sequence(1, 12).getInfo().forEach(function(monthNum) {
  ...
  var filtered = modis.map(function(img) {
    return img.updateMask(validCount.gte(3)); // Only if >=3 images
  });
  ...
  Export.image.toDrive({
    ...
    folder: 'Penang_AOD_2015', // this is the Drive folder name
