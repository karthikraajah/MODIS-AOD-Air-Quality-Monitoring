...
// === 3. QA & cloud mask function for AOD 550nm ===
function maskAOD550(image) {
  ...
  var aod550 = image.select('Optical_Depth_055').multiply(0.001);
...
ee.List.sequence(1, 12).getInfo().forEach(function(monthNum) {
  ...
  var meanAOD = filtered.mean().clip(kuching).rename('mean_AOD_550');
  ...
  print('📊 Mean AOD 550nm for ' + monthNames[monthNum - 1] + ' 2015:', stats);
  ...
  }, 'Mean AOD 550nm - ' + monthNames[monthNum - 1] + ' 2015');

  // === Export to GeoTIFF ===
  Export.image.toDrive({
    image: meanAOD,
    description: 'Kuching_AOD550_' + monthNames[monthNum - 1] + '_2015',
    folder: 'Kuching_AOD550_2015',
    fileNamePrefix: 'Kuching_AOD550_' + monthNames[monthNum - 1] + '_2015',
